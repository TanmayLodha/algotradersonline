...
SCRIPT_LIST = ['ACC', 'AUBANK', 'ADANIENT', 'ADANIPORTS', 'AMBUJACEM', 'APOLLOHOSP', 'ASIANPAINT', 'AUROPHARMA', 'AXISBANK', 'BAJAJ-AUTO', 'BAJFINANCE',
...
def login():
    access_token = AliceBlue.login_and_get_access_token(username=Credentials.UserName.value, password=Credentials.PassWord.value, twoFA=Credentials.TwoFA.value, api_secret=Credentials.SecretKey.value, app_id=Credentials.AppId.value)
    ...
    alice.subscribe([alice.get_instrument_by_symbol('NSE', i.upper()) for i in SCRIPT_LIST], LiveFeedType.MARKET_DATA)
...
def event_handler_quote_update(message):
    ...
    df = df.append({'symbol': instrument, 'timestamp': timestamp, 'vol': vol,
...
def getOHLC_df(df):
    ...
    for name, group in grouped:
        ...
        df_final.to_excel(f'/Users/nitishgupta/Desktop/ORB/{datetime.datetime.now().strftime("%Y-%m-%d")}.xlsx')
   
    
if __name__ == '__main__':
    while((datetime.datetime.now().time() <= datetime.time(9,15,00)) or (datetime.datetime.now().time() >= datetime.time(15,30,00))):
        pass

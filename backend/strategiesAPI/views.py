...
def stop_name(request):
    try:
        ...
        q = Papertrade.objects.all().filter(username=username).filter(isActive=True).filter(isCompleted=False).get(
            name=name)
...
def get_summary(request):
    try:
        ...
        return Response(
            {
                'response": "Oops. Your have given a wrong field. "delta" expected'
            },
            status=status.HTTP_400_BAD_REQUEST)
